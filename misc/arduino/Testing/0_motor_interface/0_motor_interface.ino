...
void main_loop_counter(){
  ML_counter++;
  if (ML_counter > 2000 * ML_rate){
    ML_enable = true;
    ML_counter = 0;
  }
  return;
...
float calc_wheel_speed(int counter, float sample_time){
  float sensor_freq = counter/0.1;
...
void calc_wheel_speed_LR(){
  L_current_speed = calc_wheel_speed(L_counter, ML_rate);
  R_current_speed = calc_wheel_speed(R_counter, ML_rate);
}
...
float PID(float rk, float yk, float ik_1, float ek_1, float ts, float P, float I, float D){
  float ek = rk - yk;
  float ik = ik_1 + ts * (ek + ek_1) / 2;
  ik = clamp(ik/I, -100, 100);
  float dk = (ek - ek_1) / ts;
...
void PID_LR(){
  L_duty = PID(L_target_speed, L_current_speed, L_ik_1, L_ek_1, ML_rate, PID_P, PID_I, PID_D);
  L_ik_1 = PID_ik_1;
  L_ek_1 = PID_ek_1;
  R_duty = PID(R_target_speed, R_current_speed, R_ik_1, R_ek_1, ML_rate, PID_P, PID_I, PID_D);
  R_ik_1 = PID_ik_1;
  R_ek_1 = PID_ek_1;
}
